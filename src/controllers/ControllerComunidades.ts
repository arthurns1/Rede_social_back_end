...
export class ControllerComunidades {
    static async create_comunidade(
        ...
        try {
            ...
            await pool.query(
                "INSERT INTO comunidades (id_mensagem, nome_comunidade) VALUES (DEFAULT, $1)",
                params,
            );
    ...
    static async get_comunidade_by_id(req: Request, res: Response) {
        try {
            const params = [req.params.id_comunidade];
    ...
    static async update_comunidade_by_id(req: Request, res: Response) {
        try {
            const params = [req.params.id_comunidade, req.body.nome_comunidade];

            const results = await pool.query(
                "UPDATE comunidades WHERE id_comunidade = $1 SET nome_comunidade = $2;",
                params,
            );
